...
const permissions = [
  ...
  ['users.view', 'View users'], ['users.create', 'Create users'], ['users.update', 'Edit users'],
  ['roles.view', 'View roles'], ['roles.create', 'Create roles'], ['roles.update', 'Edit roles'], ['roles.delete', 'Delete roles'],
];
  ...
  return <div className="mx-auto max-w-6xl">
    ...
    <Dialog open={isOpen} onClose={close} maxWidth="sm" fullWidth><DialogTitle sx={{ color: '#0f172a' }}>{editingRole ? 'Edit role' : 'Create role'}</DialogTitle>
      <form onSubmit={submit}><DialogContent><Stack spacing={2}>
        ...
        <div><Typography variant="subtitle2" sx={{ color: '#334155', mb: 1 }}>Permissions</Typography>{permissions.map(([value, label]) => <FormControlLabel key={value} control={<Checkbox checked={form.permissions.includes(value)} onChange={() => togglePermission(value)} />} label={label} />)}</div>
